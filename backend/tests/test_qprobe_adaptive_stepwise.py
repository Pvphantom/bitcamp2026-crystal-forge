...
from app.ml.qprobe_adaptive_step_features import (
    build_qprobe_adaptive_step_feature_vector,
    qprobe_adaptive_step_feature_dim,
)
...
from scripts.train_qprobe_adaptive_stepwise_model import split_by_operator_family


def test_step_feature_vector_has_expected_dimension() -> None:
    ...
    assert features.shape == torch.Size([qprobe_adaptive_step_feature_dim()])


def test_stepwise_split_prevents_operator_family_leakage() -> None:
    samples = []
    for family in ("hubbard", "tfim"):
        for op_family in ("local_z", "mixed_local", "pair_zz", "pair_xx"):
            for _ in range(2):
                samples.append(
                    {
                        "features": torch.zeros(qprobe_adaptive_step_feature_dim()),
                        "safe_stop": False,
                        "margin": -0.01,
