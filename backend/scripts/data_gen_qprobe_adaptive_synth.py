...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=ARTIFACTS_DIR / "qprobe_adaptive_synth_dataset.pt")
    ...
    print(f"saved {len(dataset)} adaptive synthetic ML-QProbe samples to {args.output}")
                ...
                for readout_flip_prob in NOISE_LEVELS:
                    for shots_per_group in SHOT_COUNTS:
                        for tolerance in TOLERANCES:
                            ...
                            rows.append(
                                {
                                    "features": build_qprobe_adaptive_feature_vector(
                                        problem=problem,
                                        operator_map=bundle.operator_map,
                                    ...
                                    "recommended_cost": result.final_plan.cost,
                                    "full_cost": result.full_plan.cost,
                                    "measurement_savings": result.full_plan.cost - result.final_plan.cost,
                                    "success": result.oracle_benchmark_within_tolerance,
                                    "adaptive_stop_success": result.success,
                                    "max_abs_error": result.max_abs_error,
                                    "group_bases": result.final_plan.bases,
