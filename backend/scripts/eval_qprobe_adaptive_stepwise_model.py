...
from scripts.train_qprobe_adaptive_stepwise_model import family_group_key, split_by_operator_family, evaluate_with_thresholds
...
def evaluate(*, dataset_path: Path, model_path: Path) -> dict[str, Any]:
    ...
    from scripts.train_qprobe_adaptive_stepwise_model import make_loader
...
def main() -> None:
    ...
    parser.add_argument("--dataset", type=Path, default=Path("backend/artifacts/qprobe_adaptive_stepwise_dataset.pt"))
    parser.add_argument("--model", type=Path, default=Path("backend/artifacts/qprobe_adaptive_stepwise_mlp.pt"))
    parser.add_argument("--out", type=Path, default=Path("backend/artifacts/qprobe_adaptive_stepwise_eval.json"))
