...
def train(
    *,
    ...
    if family_filter is not None:
        routing_samples = [
            sample for sample in routing_samples
            if str(sample.get("problem_metadata", {}).get("family")) == family_filter
        ]
        intrinsic_samples = [
            sample for sample in intrinsic_samples
            if str(sample.get("problem_metadata", {}).get("family")) == family_filter
        ]
        benchmark_samples = [
            sample for sample in benchmark_samples
            if str(sample.get("problem", {}).get("model_family")) == family_filter
        ]
    ...
    intrinsic_weights = torch.tensor(
        [1.0 / intrinsic_counts.get(label, 1) for label in INTRINSIC_RISK_LABELS],
        dtype=torch.float32,
    )
    ...
    for _ in range(epochs):
        ...
        benchmark_report = evaluate_regime_benchmark(samples=benchmark_6x6, model_path=temp_model_path)
        benchmark_acc = float(benchmark_report["summary"]["overall_accuracy"])
        tfim_q_acc = float(
            benchmark_report["summary"]["by_truth_label"].get("quantum_frontier", {}).get("accuracy", 0.0)
        )
        score = benchmark_acc + 0.75 * tfim_q_acc
